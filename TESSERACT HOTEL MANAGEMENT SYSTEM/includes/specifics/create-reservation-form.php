<center>
	<div class="guest-registeration" id="form">
			...
			if (empty($errors)==true)
				{	
					if (submit_btn_clicked('create-reservation-button')) {
						echo"<h2 style='color:green'>Reservation Created </h2>";		
						$guest->bookDownRoom();	// creating reservation 
						header("Location:create-reservation.php?success");
						exit();
					}
				} else {
		...
		<form id="GUEST-RESERVATION" action="create-reservation.php" method="POST">
			...
			<div class="inner">
				<div class="left-side">
					...
					<span class="field">Room Number <b id="mandatory-star">*</b></span> 
						<select name="room_id" id="vacant_room_search_RESERVATION" >
							<option value="#"> --Choose--</option>
							<?php $room->optionsOfAllRooms('room_id');?>
						</select>
				...
				<div class="right-side">
					<span class="field"> Check In Date </span> <input type="date" name="checkIn_date" id="checkIn_date" min="<?php echo $today ?>"  value="<?php postConst('checkOut_date'); ?>"  max="<?php echo $today_by_next_year; ?>"  disabled="disabled" class="disabled-date" /> <!-- Check In Date Input -->
			...
			<div>
				<input type="submit" name="create-reservation-button" value="Create Reservation" >
			</div>
		</form>		
