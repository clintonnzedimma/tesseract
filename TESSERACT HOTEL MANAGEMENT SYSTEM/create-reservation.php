...
<?php
if (submit_btn_clicked('create-reservation-button')) {
	...
	if (empty($checkIn_date)) {
		$errors[]="$icon_error Please Input check in date";
	}
	...
	if ($guest->generalGuestsDateRangeCheck($room_id, $checkIn_date) || $guest->generalReservationsDateRangeCheck($room_id, $checkIn_date) && !empty($checkIn_date)) {
		$errors[]="$icon_error The Date You would like to check in is not available ";
	}
	if ($guest->generalGuestsDateRangeCheck($room_id, $checkOut_date) ||  $guest->generalReservationsDateRangeCheck($room_id, $checkOut_date) && !empty($checkOut_date)) {
		$errors[]="$icon_error The Date You would like to check out is not available ";
	}	
...
if (!isset($_GET['success'])) {
	include 'includes/specifics/create-reservation-form.php';
} else {
	include 'includes/specifics/create-reservation-success.php';
}
?>
	...
	$(function(){
		...
		$('select[name=room_id]').on('change',function (){ 
			...
			if ($(this).val()==null || $(this).val()=="#" ) {
				...
				* disable check in & check out form
				* set amount paid as false
				*/


				 $('input[name=checkIn_date]').attr("disabled", "disabled"); 
				 $('input[name=checkIn_date]').attr("class", "disabled-date");
				 $('input[name=checkIn_date]').val(false);
				...
				$('input[name=checkIn_date]').attr("disabled", false);
				 $('input[name=checkIn_date]').attr("class", false); 				
		...
		$('input[name=checkIn_date]').on('change', function (){
			$("input[name=checkOut_date").attr("min", $(this).val());
				dateInRangeCheckViaJSON(); // checking if date has been taken and sending error message
		...
		$('input[name=checkOut_date]').on('change', function () {
					dateOutRangeCheckViaJSON(); // checking if date has been taken and sending error message
					
					// if number of check in and check ot date are the same, numOfDays equals 1	
					var numOfDays=($('input[name=checkIn_date]').change().val()==$(this).change().val()) ? 1 :daysBetween($('input[name=checkIn_date]').change().val(), $(this).change().val() );
...
function dateInCheck (selector) {
	var date=$(selector).val();
	if (date.length==0) {
		$("#checkIn_date-ERROR").html(icon_error+"Please Input Check In Date");
		$("#checkIn_date-ERROR").show();
		return true;
	} else {
		$("#checkIn_date-ERROR").hide();
		return false;
	}
}


function dateOutCheck (selector) {
	var date=$(selector).val();
	if (date.length==0) {
...
function dateOutRangeStatus() {
	if ($("[PHPtoJs=checkOut_date_range-STATUS]").html()==1 && $('input[name=checkOut_date]').val().length>0) {
		...
		return false;
		 ...
		 $("#GUEST-RESERVATION").submit(function (e) {
		 	checkSex();
			...
			dateInCheck('input[name=checkIn_date]');
			dateOutCheck('input[name=checkOut_date]');
			...
			dateOutRangeCheckViaJSON();
		 	...
		 	if (checkFullName("input[name=full_name]")  || checkPhoneNumber("input[name=phone]") || checkSex() || checkEmail("input[name=email]") || checkRoomField("select[name=room_id] option:selected")  || dateInCheck('input[name=checkIn_date]') || dateOutCheck('input[name=checkOut_date]')|| dateInRangeStatus() || dateOutRangeStatus()) {
